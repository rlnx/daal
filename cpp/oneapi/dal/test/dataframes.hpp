...
#define GENERATE_DATAFRAME(...) \
    GENERATE(as<oneapi::dal::test::dataframe_builder>{}, __VA_ARGS__).build()
...
public:
    ...
    const array<double>& get_cached_column_means() const {
        return column_means_cache_;
    }

    void update_cached_column_means(const array<double>& means) {
        column_means_cache_ = means;
    }

    const array<double>& get_cached_column_variances() const {
        return column_variances_cache_;
    }

    void update_cached_column_variances(const array<double>& variances) {
        column_variances_cache_ = variances;
    }
...
private:
    ...
    array<double> column_means_cache_;
    array<double> column_variances_cache_;
};
...
public:
    ...
    dataframe fill_column(std::int64_t column_idx, Op&& op) const {
        return mutate_impl([&](dataframe_impl& impl) {
            const std::int64_t row_count = get_row_count();
            const std::int64_t column_count = get_column_count();
            float* data = impl.get_data();
            for (std::int64_t i = 0; i < row_count; i++) {
                data[i * column_count + column_idx] = op(i);
            }
        });
    }

    array<double> column_means() const;

    array<double> column_variances() const;
