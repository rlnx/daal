...
inline void sym_eigval(ndview<Float, 2, order>& data_or_eigvecs, ndview<Float, 1>& eigvals) {
    ...
    sym_eigval_impl(data_or_eigvecs.get_mutable_data(),
...
/// descending order determined by eigenvalues. For more details, see `sym_eigval`.
template <typename Float, ndorder order>
inline void sym_eigval_descending(ndview<Float, 2, order>& data_or_eigvecs,
                                  ndview<Float, 1>& eigvals) {
    sym_eigval(data_or_eigvecs, eigvals);
